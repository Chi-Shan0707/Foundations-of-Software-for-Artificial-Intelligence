...
> GPU 显存层次结构（Global Memory、Shared Memory、Register），Triton 编程模型对底层硬件的抽象，向量化乘法、矩阵乘法与 LeNet-5 全连接层替换。
...
> 共享内存在算子优化中扮演关键角色。 由于 Global Memory 太慢而寄存器太分散（线程私有），共享内存提供了一个平衡点：足够快、又可以被同 Block 内的线程协同读写。
