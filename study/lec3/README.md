...
### A.7 Python 字节码缓存：`__pycache__`

#### A.7.1 什么是 `__pycache__`

`__pycache__` 是 Python 3 自动生成的缓存目录，用于存储编译后的字节码文件（`.pyc`）。
...
**命名规则**：`模块名.cpython-版本号.pyc`
- `cpython-310`：表示 CPython 3.10 版本
- 不同版本的 Python 生成的字节码不兼容，会并存
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  第一次运行：python main.py                                     │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  1. 检查 __pycache__/main.cpython-310.pyc 是否存在      │   │
│  │     结果：不存在                                          │   │
│  ├─────────────────────────────────────────────────────────┤   │
│  │  2. 读取 main.py 源码                                    │   │
│  ├─────────────────────────────────────────────────────────┤   │
...
│  │  4. 执行字节码                                            │   │
│  ├─────────────────────────────────────────────────────────┤   │
│  │  5. 将字节码写入 __pycache__/main.cpython-310.pyc       │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  第二次运行：python main.py                                     │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  1. 检查 __pycache__/main.cpython-310.pyc 是否存在      │   │
│  │     结果：存在                                            │   │
│  ├─────────────────────────────────────────────────────────┤   │
...
| 语言 | 编译产物 | 何时生成 | 是否缓存 | 是否版本相关 |
|------|---------|---------|---------|-------------|
| **Python** | `.pyc` | 第一次运行时 | ✅ `__pycache__` | ✅ |
| **Java** | `.class` | `javac` 编译时 | ❌ 需手动管理 | ✅ |
| **C/C++** | `.o`/`.so` | `gcc` 编译时 | ❌ 需手动管理 | ❌ |
| **Go** | 可执行文件 | `go build` 时 | ❌ | ❌ |
...
1. **自动缓存**：Python 自动将编译后的字节码保存到 `__pycache__`
2. **版本隔离**：不同 Python 版本生成的字节码会共存
3. **惰性更新**：只有源文件修改时才重新编译
4. **非必需**：`.pyc` 文件可以随时删除，不影响程序运行
5. **性能优化**：缓存字节码可以减少启动时的编译开销
